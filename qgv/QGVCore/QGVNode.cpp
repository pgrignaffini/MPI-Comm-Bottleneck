...
void QGVNode::setRecv_size(std::vector<int> entry)
{
    m_recv_size = entry;
}

void QGVNode::setRecv_count(std::vector<int> entry)
{
    m_recv_count = entry;
}

void QGVNode::setSend_size(std::vector<int> entry)
{
    m_send_size = entry;
}

void QGVNode::setSend_count(std::vector<int> entry)
{
    m_send_count = entry;
}

int QGVNode::getRecv_size()
{
    int sum = 0;
    for(int i = 0; i < m_recv_size.size(); i++)
        sum += m_recv_size[i];

    return sum;
}

int QGVNode::getRecv_count()
{
    int sum = 0;
    for(int i = 0; i < m_recv_count.size(); i++)
        sum += m_recv_count[i];

    return sum;
}

int QGVNode::getSend_size()
{
    int sum = 0;
    for(int i = 0; i < m_send_size.size(); i++)
        sum += m_send_size[i];

    return sum;
}

int QGVNode::getSend_count()
{
    int sum = 0;
    for(int i = 0; i < m_send_count.size(); i++)
        sum += m_send_count[i];
